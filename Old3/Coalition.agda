...
CoalitionOfWhole : (m : ℕ) → Σ (ProtoCoalition m) λ p → (Split-Coalitions p empty × (∀ n → (m ℕ.> n) → InCoalition m n p))
CoalitionOfWhole zero = empty , (record
   { coalition-1 = EmptyCoalition 0 ; coalition-2 = EmptyCoalition 0 ; disj = l-empty-disjoint empty ; comp = empty-complete }) 
   , λ {n ()}
CoalitionOfWhole (suc m) with CoalitionOfWhole m    
... | fst , split , comp = c-cons m (ℕProp.n<1+n m) (Lift fst) 
                          , ExpandCoalition m fst empty split 
                          , λ {n (s≤s x) → Whole-is-Complete m n fst x comp} where
    Whole-is-Complete : (m n : ℕ) → (fst : ProtoCoalition m) → (n ℕ.≤ m) → ((n₁ : ℕ) → suc n₁ ℕ.≤ m → InCoalition m n₁ fst) → InCoalition (suc m) n (c-cons m (ℕProp.n<1+n m) (Lift fst))
    Whole-is-Complete m n fst n≤m f with n ℕ.≟ m 
    ... | false because ofⁿ ¬p = in-coalition-tail m n (ℕProp.n<1+n m) (Lift fst) (idxInCoalitionLift fst (f n (ℕProp.≤∧≢⇒< n≤m ¬p))) 
    ... | true because ofʸ p rewrite p = in-coalition-head m (ℕProp.n<1+n m) (Lift fst)
