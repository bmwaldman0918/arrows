...
AnyP→¬AllCP : ∀ {a b} → {A : Set a} → {P : Pred A b} → {xs : List A} → Any P xs → ¬ All (∁ P) xs
AnyP→¬AllCP (here px) (cpx All.∷ _) = cpx px 
AnyP→¬AllCP (there any-P) (_ All.∷ all-C-P) = AnyP→¬AllCP any-P all-C-P

AnyCP→¬AllP : ∀ {a b} → {A : Set a} → {P : Pred A b} → {xs : List A} → Any (∁ P) xs → ¬ All P xs
AnyCP→¬AllP (here px) (px' All.∷ _) = px px'
AnyCP→¬AllP (there p) (_ All.∷ p') = AnyCP→¬AllP p p'

AllP→¬AnyCP : ∀ {a b} → {A : Set a} → {P : Pred A b} → {xs : List A} → All P xs → ¬ Any (∁ P) xs
AllP→¬AnyCP (px All.∷ _) (here px') = px' px
AllP→¬AnyCP (_ All.∷ p) (there p') = AllP→¬AnyCP p p'

AllCP→¬AnyP : ∀ {a b} → {A : Set a} → {P : Pred A b} → {xs : List A} → All (∁ P) xs → ¬ Any P xs
AllCP→¬AnyP (px All.∷ _) (here px') = px px'
AllCP→¬AnyP (_ All.∷ p) (there p') = AllCP→¬AnyP p p'

¬AnyP→AnyCP : ∀ {a b} → {A : Set a} → {P : Pred A b} → {xs : List⁺ A} → (Decidable P) → ¬ Any P (toList xs) → Any (∁ P) (toList xs)
...
¬AllP→AnyCP : ∀ {a b} → {A : Set a} → {P : Pred A b} → {xs : List⁺ A} → (Decidable P) → ¬ All P (toList xs) → Any (∁ P) (toList xs)
